...
class ContextPacketAuthorityTests(unittest.TestCase):
    ...
    def test_unrestricted_node_preserves_legacy_buffer_semantics(self):
        spec = SimpleNamespace(
            id="worker",
            name="Worker",
            input_keys=[],
        ...
        packet = build_node_context_packet(
            node_spec=spec,
            buffer=FakeBuffer({"brief": "ok"}),
            goal_context="goal",
        )
        self.assertEqual(["brief"], [entry.key for entry in packet.entries])
    ...
    def test_nested_optional_credential_field_omits_whole_value(self):
        secret = "credential-value-must-not-appear"
        packet = build_context_packet(
            node_id="worker",
            node_name="Worker",
            goal_context="goal",
            values={"integration_config": {"endpoint": "x", "auth": {"api_key": secret}}},
    ...
    def test_nested_credential_inside_array_is_fenced(self):
        packet = build_context_packet(
            node_id="worker",
            node_name="Worker",
            goal_context="goal",
            values={"records": [{"safe": 1}, {"credentials": {"user": "u", "password": "p"}}]},
            context_keys=["records"],
        )
        self.assertEqual("sensitive_value", packet.omissions[0].reason)
        self.assertNotIn('"password"', render_context_packet(packet))
